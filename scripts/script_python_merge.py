...
df1_mtbseq_renamed=df1_mtbseq.rename(columns={'Pos':'POS', 'Ref':'REF', 'Allel':'ALT', 'Freq':'Freq_Mutated_MTBSEQ', 'Cov':'DP_MTBSEQ', 'Subst':'EVENT_MTBSEQ', 'Gene':'GENEID', 'GeneName':'GENE', 'Product': 'Product_MTBSEQ','ResistanceSNP':'Res_MTBseq',  'PhyloSNP':'PhyloSNP_MTBSEQ',  'InterestingRegion' : 'ANTIBIO'})
...
df1_mtbseq_renamed['ANTIBIO']=df1_mtbseq_renamed['ANTIBIO'].apply(lambda x: change_drug_to_abbreviation(col_split(x).capitalize())[0]  if x !=" " else "NA")
...
def validate_tbProf_resist(x):
    #if str(x[18]) != "nan":
        #          nan      3841652  nan        Na          c.-232A>T  nan        A-232T        NA       nan
        #print(str(x[0]), str(x[1]), str(x[8]), str(x[10]), str(x[16]), str(x[17]),str(x[18]), str(x[19]),str(x[21]))
        #                           C333G   Na            p.Cys333Gly nan    NA             C333G
        #                   nan    Na             c.33A>G    nan    A33G            NA
        #                   G594E  Na             p.Gly594Glu nan    NA            G594E
                          #  V381G     Na       p.Val381Gly    nan        NA          V381G
                        #    A5G       Na       p.Ala5Gly      nan        NA          A5G
    if str(x[16])=="nan":
        return " "
    elif (str(x[17])=="nan" and str(x[19])==str(x[8])) or ( str(x[17])=="nan" and str(x[18])==str(x[10])):
        return "ND"
...
def validate_mtbseq_resist(x):
    ...
    if  str(x[21])=="nan":
        return " "
    if "phylo" in str(x[21]):
        return x[21]
    elif change_drug_to_abbreviation(str(x[21]).split(' ')[0].capitalize())[0]==x[0] :
        return "Res"
    else:
        return str(x[21])+" "+x[0]+" "+"Not relevant"
...
merge_after_drop=merge_after_drop.drop(columns=['INDEXING', 'DRUG' , 'HGVS_C_TB' , 'HGVS_P_TB' , 'EVENT_TB', 'ANTIBIO_TB'])
